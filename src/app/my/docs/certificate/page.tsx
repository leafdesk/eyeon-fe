...
export default function CertificatePage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        setIsLoading(true)
        setError(null)
        const response = await api.form.getList(FORM_TYPES.CERTIFICATE)
        ...
        console.error('재직증명서 목록 조회 오류:', err)
        setError(
          '재직증명서 목록을 불러오는 데 실패했습니다. 다시 시도해주세요.',
        )
      } finally {
        setIsLoading(false)
      }
  ...
  const handleDocClick = (id: number) => {
    router.push(`/my/docs/certificate/${id}`)
  }
  ...
  return (
    <main className="min-h-screen bg-[#0F1626] text-white">
      ...
      <div className="px-6 pt-9 pb-6">
        <h1 className="text-[28px] font-semibold">재직증명서</h1>
      </div>
